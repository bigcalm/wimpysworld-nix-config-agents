...
#
# This is a reviewed local copy of the wrapper deployed by the nix-config
# repository (home-manager/_mixins/development/github/gh-api-safe.sh),
# patched to also reject --hostname in Phase 2, matching the glab wrapper.
# Keep this file in sync with upstream when the source changes.
#
...
#
# Policy summary:
#   * Argv pre-check rejects -X/--method, --hostname, -f/--field,
#     -F/--raw-field, and --input on every token. The only exception is
#     `-f query=...` (or -F/--field/--raw-field with `query=`) when the
#     endpoint is `graphql`.
...
POLICY
    Argv pre-check rejects -X/--method, --hostname, -f/--field,
    -F/--raw-field, and --input on every token. The exception is
    `-f query=...` (or the equivalent -F/--field/--raw-field forms)
    when the endpoint is `graphql`, so read-only GraphQL queries
    remain usable.
